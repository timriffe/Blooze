...
extrap <- function(DX, NX, to = 130){
	DX <- DX[-nrow(DX), ]
	NX <- NX[-nrow(NX), ]
	mat110 <- matrix(NA,ncol=ncol(DX),nrow=(to+1),dimnames=list(0:to,colnames(DX)))
	DXe <- mat110
	DXe[rownames(DX),colnames(DX)] <- DX
	NXe <- mat110
	NXe[rownames(NX),colnames(NX)] <- NX
	
	ages.i <- rep(91,ncol(DX)) + 1
	ltper_mx_v5(DXe,NXe,ages.i,fit.from.i = 81 )
}
